...
class ExternalDoctorListScreen extends StatefulWidget {
  ...
  _ExternalDoctorListScreenState createState() =>
      _ExternalDoctorListScreenState();
}

class _ExternalDoctorListScreenState extends State<ExternalDoctorListScreen> {
  ...
  Widget build(BuildContext context) {
    // Get the screen width to determine if we need responsive layout
    final screenWidth = MediaQuery.of(context).size.width;
    final isSmallScreen = screenWidth < 1100; // Threshold for small screens
  ...
  Widget _buildHeader(bool isSmallScreen) {
    // For small screens, stack the title and buttons vertically
    if (isSmallScreen) {
      return Container(
              ...
              children: [
                ...
                Expanded(
                  child: Text(
                    ...
                    style: _headerStyle.copyWith(fontSize: 22),
    ...
    // For normal screens, use the regular layout
    return Container(
      padding: const EdgeInsets.fromLTRB(24, 24, 24, 16),
      child: Row(
  ...
  Widget _buildFilters(bool isSmallScreen) {
    // For small screens, stack the search and filter vertically
    if (isSmallScreen) {
      return Container(
        ...
        child: Column(
          ...
          children: [
            ...
            _buildSpecialtyFilter(true), // Set to full width
          ],
        ),
      );
    }
    ...
    return Container(
      ...
      child: Row(
        children: [
          ...
          const SizedBox(width: 16),
          _buildSpecialtyFilter(false), // Normal width
        ],
      ),
    );
  }
  ...
  Widget _buildSpecialtyFilter(bool fullWidth) {
    return Container(
      height: 48,
  ...
  Widget _buildAddDoctorButton() {
    return ElevatedButton.icon(
      ...
      onPressed: () {
        // Navigate through sidebar instead of direct navigation
        // This will select the doctor registration screen in the sidebar
        _navigateToExternalDoctorRegister();
      },
    );
  }

  void _navigateToExternalDoctorRegister() {
    // Access the ExternalSideBar state and update the selected index
    // We need to find the nearest ExternalSideBar ancestor and update its state
    final scaffold = Scaffold.of(context);
    if (scaffold.hasDrawer) {
      scaffold.openDrawer(); // Open drawer if available
    } else {
      // If not using drawer, we can use a global navigation approach through a state management solution
  ...
  // Full width data table for larger screens
  ...
  Widget _buildResponsiveDataTable() {
    return ListView.separated(
      ...
      itemBuilder: (context, index) {
        ...
        return _buildResponsiveDoctorCard(doctor);
      },
    );
  }

  // Card-style row for small screens
  Widget _buildResponsiveDoctorCard(Doctor doctor) {
    return Card(
